...
export default function AdminDashboardModal({ isOpen, onClose }: AdminDashboardModalProps) {
  ...
  const handlePrintReport = () => {
    ...
    if (activeTab === "bookings_report") {
      ...
      // Split totals by currency, and only count non-cancelled bookings towards revenue.
      const activeBookings = filteredBookings.filter((b) => b.status !== "cancelled");
      const totalPKR = activeBookings
        .filter((b) => (b.country?.currency || "PKR") === "PKR")
        .reduce((sum, b) => sum + (b.country?.fee || 0), 0);
      const totalUSD = activeBookings
        .filter((b) => b.country?.currency === "USD")
        .reduce((sum, b) => sum + (b.country?.fee || 0), 0);
      ...
      contentHtml = `
        <div style="margin-bottom: 16px; background: #f8fafc; border: 1px solid #cbd5e1; padding: 12px 16px; border-radius: 8px; font-size: 13px; color: #334155;">
          ...
          <strong>Total Fees (excl. cancelled):</strong> PKR ${totalPKR.toLocaleString()} + USD ${totalUSD.toLocaleString()}
        </div>
        <table style="width: 100%; border-collapse: collapse; font-size: 12px; font-family: sans-serif;">
          <thead>
            <tr style="background-color: #142127; color: white;">
              ...
              <th style="padding: 10px; border: 1px solid #cbd5e1; text-align: left;">Mode & City/Country</th>
              <th style="padding: 10px; border: 1px solid #cbd5e1; text-align: left;">Date & Time</th>
              <th style="padding: 10px; border: 1px solid #cbd5e1; text-align: right;">Fee</th>
          ...
          : filteredBookings
            .map((b, idx) => {
              ...
              <tr style="background-color: ${idx % 2 === 0 ? "#ffffff" : "#f8fafc"};">
                ...
                <td style="padding: 8px 10px; border: 1px solid #e2e8f0;">${b.mode === "physical" ? "In-Person Clinic" : "Online Video"} (${cityOrCountry})</td>
                <td style="padding: 8px 10px; border: 1px solid #e2e8f0;">${b.selectedDate} ${b.selectedSlot?.label || ""}</td>
                <td style="padding: 8px 10px; border: 1px solid #e2e8f0; text-align: right; font-weight: bold;">${feeDisplay}</td>
                          ...
                          filteredBookings.map((item) => (
                            <tr key={item.id} className="hover:bg-clinical-50/60 transition-colors">
                              ...
                              <td className="p-3 capitalize">
                                ...
                                <div className="text-[10px] text-clinical-500 mt-0.5">
                                  {item.mode === "online" ? (item.country?.name || "Unknown Country") : item.location}
                                </div>
                              </td>
                              <td className="p-3">
