...
export default function AdminDashboardModal({ isOpen, onClose }: AdminDashboardModalProps) {
  ...
  const handlePrintReport = () => {
    ...
    if (activeTab === "bookings_report") {
      ...
      const totalFees = filteredBookings.reduce((sum, b) => sum + (b.country?.fee || 0), 0);
      contentHtml = `
          ...
          : filteredBookings
            .map((b, idx) => {
              ...
              <tr style="background-color: ${idx % 2 === 0 ? "#ffffff" : "#f8fafc"};">
                ...
                <td style="padding: 8px 10px; border: 1px solid #e2e8f0;">${b.mode === "physical" ? "In-Person Clinic" : "Online Video"} (${b.location || "Online"})</td>
                <td style="padding: 8px 10px; border: 1px solid #e2e8f0;">${b.selectedDate} ${b.selectedSlot?.label || ""}</td>
                <td style="padding: 8px 10px; border: 1px solid #e2e8f0; text-align: right; font-weight: bold;">${(b.country?.fee || 0).toLocaleString()}</td>
